...
Page({
  ...
  getdetails(){
    ...
    requestUtil.Requests('activity/giftPacks/queryDetail', params).then((res) => {
  ...
  getguige(){
    ...
    requestUtil.Requests('activity/advertise/queryStock', params).then((res) => {
 ...
 tocart() {
  ...
  handlePay(){
    ...
    // }else{
      ...
      wx.navigateTo({
        url: '../activitylb-order/index?activityNo='+activityNo+'&stockNo='+stockNo+'&activityOrderNo='+activityOrderNo
      })
    // }
  },
  //去下单
  goxiadan(e){
    ...
    wx.navigateTo({
      url: '../activitydy-order/index?activityNo='+activityNo+'&stockNo='+stockNo+'&activityOrderNo='+activityOrderNo
    })
  },
  /**
   * 生命周期函数--监听页面初次渲染完成
    ...
    if (res.from === 'button') {
      ...
      if (_this.data.activityType != '4') {
        return {
          ...
          path: '/pages/start/start?userId=' + shareId + '&url=/pages/index-child/pages/activitylb-details/index&activityNo=' + _this.data.activityNo,
          imageUrl:img,
        }
      } else {
        return {
