...
Page({
  ...
  onLoad: function (options) {
    console.log(options)
    if(options.activityOrderNo=="undefined"){
      var shenfen=1;//这是发起者
    }else{
      var shenfen=2;//这是参与者
    }
  ...
  getxiangqing(){
    ...
    console.log(params)
    requestUtil.Requests('activity/chains/querySubmitOrderDetail', params).then((res) => {
  ...
  submitOrder(){
    ...
    console.log(that.data.activityOrderNo)
    if(that.data.activityOrderNo!="undefined"){ 
      console.log("你是参与者")
      let params = {
        activityNo:that.data.activityNo,
        stockNo:that.data.stockNo,
        price:that.data.data.connectPrice,
      ...
      console.log(params)
      requestUtil.Requests('activity/chains/participateActivity', params).then((res) => {
        console.log(res)
        if(res.flag){
      ...
      let params ={
        activityNo:that.data.activityNo,
        stockNo:that.data.stockNo,
        price:that.data.data.faucetPrice,
        num:that.data.data.num,
        userId:wx.getStorageSync('userId'),
        receiver:that.data.addressData.persion,
        telephone:that.data.phone,
      ...
      console.log(params)
      requestUtil.Requests('activity/chains/launchActivity', params).then((res) => {
  ...
  handleActivityPay(dataNo) {
    ...
    let payPrice=that.data.data.faucetPrice;
    ...
    console.log(params)
    requestUtil.Requests('activity/chains/payLaunchActivity', params).then((res) => {
      console.log(res)
      ...
      wx.requestPayment({
        ...
        success: function(res) {
            wx.redirectTo({
              url: '/pages/child/pages/order-success/order-success?shopName=' + shopName + '&payPrice=' + payPrice + '&activityNo=' + activityNo + '&activityType=' + '4' + '&cartNo=' + dataNo + '&shopType=' + shopType + '&shopNo=' + shopNo + '&len=1'+'&img='+img+'&type='+shopType+'&shopname='+that.data.data.activityName+"&isshare=1",
            })
        },
        fail: function() {
          wx.showToast({
  ...
  handleActivityPays(dataNo,activityOrderNo) {
    ...
    let shopType=that.data.data.shopDTO.shopType;
    let shopNo=that.data.data.shopDTO.shopNo;
    let payPrice=that.data.data.connectPrice;
    let activityNo=that.data.data.activityNo;
    let img=that.data.data.imgPath
    let params={
    ...
    console.log(params)
    requestUtil.Requests('activity/chains/payParticipateActivity', params).then((res) => {
      console.log(res)
      var obj = JSON.parse(res.data)
      wx.requestPayment({
        timeStamp: obj.timeStamp,
        ...
        success: function(res) {
            wx.redirectTo({
              url: '/pages/child/pages/order-success/order-success?shopName=' + shopName + '&payPrice=' + payPrice + '&activityNo=' + activityNo + '&activityType=' + '4' + '&cartNo=' + dataNo + '&shopType=' + shopType + '&shopNo=' + shopNo + '&len=1'+'&img='+img+'&type='+shopType+'&shopname='+that.data.data.activityName+"&isshare=2"+'&orderNo='+activityOrderNo,
            })
        },
        fail: function() {
          wx.showToast({
