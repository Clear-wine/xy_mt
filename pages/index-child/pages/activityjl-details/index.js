...
Page({
  ...
  getdetails(){
    ...
    console.log("上传服务器的参数",params)
    requestUtil.Requests('activity/chains/queryDetail', params).then((res) => {
  ...
  getguige(){
    ...
    requestUtil.Requests('activity/chains/queryStock', params).then((res) => {
 ...
 tocart() {
  ...
  handlePay(){
    ...
    if(data.multi){
      ...
      wx.navigateTo({
        url: '../activityjl-order/index?activityNo='+activityNo+'&stockNo='+stockNo+'&activityOrderNo='+activityOrderNo
      })
    }
  },
  //去下单
    ...
    if (res.from === 'button') {
      ...
      if (_this.data.activityType != '4') {
        return {
          ...
          path: '/pages/start/start?userId=' + shareId + '&url=/pages/index-child/pages/activityjl-details/index&activityNo=' + _this.data.activityNo,
          imageUrl:img,
        }
      } else {
        return {
