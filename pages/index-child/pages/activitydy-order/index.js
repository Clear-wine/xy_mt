...
Page({
  ...
  getxiangqing(){
    ...
    console.log(params)
    requestUtil.Requests('activity/advertise/querySubmitOrderDetail', params).then((res) => {
  ...
  submitOrder(){
    ...
    // }else{
      let params = {
        activityNo:that.data.activityNo,
        stockNo:that.data.stockNo,
        price:that.data.data.originalPrice,
        num:that.data.data.num,
        userId:wx.getStorageSync('userId'),
        shareUserId:wx.getStorageSync('shareUserId'),
        receiver:that.data.addressData.persion,
      ...
      console.log(params)
      requestUtil.Requests('activity/advertise/launchActivity', params).then((res) => {
        console.log(res)
        if(res.flag){
  ...
  handleActivityPay(dataNo) {
    ...
    let payPrice=that.data.data.originalPrice;
    ...
    console.log(params)
    requestUtil.Requests('activity/advertise/payLaunchActivity', params).then((res) => {
      console.log(res)
      ...
      wx.requestPayment({
        ...
        success: function(res) {
            wx.redirectTo({
              url: '/pages/child/pages/order-success/order-success?shopName=' + shopName + '&payPrice=' + payPrice + '&activityNo=' + activityNo + '&activityType=' + '3' + '&cartNo=' + dataNo + '&shopType=' + shopType + '&shopNo=' + shopNo + '&len=1'+'&img='+img+'&type='+shopType+'&shopname='+that.data.data.activityName+"&isshare=1",
            })
        },
        fail: function() {
          wx.showToast({
