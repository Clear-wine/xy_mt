...
Page({
  ...
  getdetails(){
    ...
    requestUtil.Requests('activity/advertise/queryDetail', params).then((res) => {
      console.log("商品详情====>",res)
      let detailh5 = res.data.descText
        WxParse.wxParse('detailh5', 'html', detailh5, that, 0);
        if(wx.getStorageSync('shareUserId').length==0){
          //是可以代言者
          if(res.data.represent==1){
            var danyan=1;
          }else{
            var danyan=2;
          }
        }else{
          var danyan=2;//是被分享者无法代言
        }
        console.log("是否可以代言",danyan)
      that.setData({
        data:res.data,
        danyan:danyan
      })
      ...
      if (res.from === 'button') {
        ...
        if (_this.data.activityType != '4') {
          return {
            ...
            path: '/pages/start/start?userId=' + shareId + '&url=/pages/index-child/pages/activitydy-details/index&activityNo=' + _this.data.activityNo,
            imageUrl:img,
          }
        } else {
          return {
