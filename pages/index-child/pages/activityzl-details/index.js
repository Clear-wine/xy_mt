...
Page({
  ...
  onLoad: function (options) {
    console.log(options.activityNo,options.activityOrderNo)
    this.setData({
  ...
  getdetails(){
    ...
    requestUtil.Requests('activity/help/queryDetail', params).then((res) => {
  ...
  getguige(){
    ...
    requestUtil.Requests('activity/help/queryStock', params).then((res) => {
 ...
 tocart() {
  ...
  handlePay(){
    ...
    if(data.multi){
      ...
      wx.navigateTo({
        url: '../activityzl-order/index?activityNo='+activityNo+'&stockNo='+stockNo+'&activityOrderNo='+activityOrderNo
      })
    }
  },
  //去下单
    ...
    if (res.from === 'button') {
      ...
      if (_this.data.activityType != '4') {
        return {
          ...
          path: '/pages/start/start?userId=' + shareId + '&url=/pages/index-child/pages/activityzl-details/index&activityNo=' + _this.data.activityNo,
          imageUrl:img,
        }
      } else {
        return {
