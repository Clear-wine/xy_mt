...
Page({
  ...
  getxiangqing(){
    ...
    console.log(params)
    requestUtil.Requests('activity/giftPacks/querySubmitOrderDetail', params).then((res) => {
      console.log("商品详情====>",res)      
      let list=res.data.shopList;
      list.forEach(item => {
        var prices=item.giftList.reduce((p,e)=>p+e.giftPrice,0);
        item.prices=prices;
      });
      console.log(list)
      that.setData({
  ...
  submitOrder(){
    ...
    let shippingAddress=that.data.addressData.province+that.data.addressData.city+that.data.addressData.area+that.data.addressData.address
      ...
      console.log(params)
      requestUtil.Requests('activity/giftPacks/launchActivity', params).then((res) => {
        console.log(res)
        if(res.flag){
  ...
  handleActivityPay(dataNo) {
    ...
    console.log(params)
    requestUtil.Requests('activity/giftPacks/payLaunchActivity', params).then((res) => {
      console.log(res)
      ...
      wx.requestPayment({
        ...
        success: function(res) {
            wx.redirectTo({
              url: '/pages/child/pages/order-success/order-success?&payPrice=' + payPrice + '&activityNo=' + activityNo + '&activityType=' + '2' + '&cartNo=' + dataNo + '&len=1'+'&img='+img+"&isshare=1"+'&shopType=1',
            })
        },
        fail: function() {
          wx.showToast({
